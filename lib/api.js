...
const postFields = `
  ...
  category,
  body,
  'author': author->{name, 'avatar': avatar.asset->url, body},
`;
...
export async function getProjectAndMoreProjects(slug, preview) {
  ...
  const [post, morePosts] = await Promise.all([
    curClient
      .fetch(
        `*[_type == "projects" && slug.current == $slug] | order(_updatedAt desc) {
        ${projectFields}
        body,
      }`,
        { slug }
      ...
      .then((res) => res?.[0]),
    curClient.fetch(
      `*[_type == "projects" && slug.current != $slug] | order(publishedAt desc, _updatedAt desc){
        ${projectFields}
        body,
      }[0...3]`,
      { slug }
